...
5. **Emails:** To send the **quote** email (on `POST /booking-requests/quote`), **approval / signing**, **decline**, and related notices, set `SMTP_HOST`, `SMTP_FROM`, and usually `SMTP_USER` / `SMTP_PASSWORD` (TLS on port 587 by default). If SMTP is unset, those emails are skipped but the API still records bookings and declines. Submitting a booking does not send a second pricing email (the quote email is the customer’s pricing copy). **Staff workflow:** with SMTP enabled, **“booking needs approval”** (after the customer finishes document upload) and **“booking ready to confirm”** (rental paid, deposit if required, agreement signed) go to **`ADMIN_NOTIFICATION_EMAIL`** if set; otherwise to **`SMTP_USER`** when it looks like an email, else the address parsed from **`SMTP_FROM`** (including `Name <addr@host>`). Set **`ADMIN_NOTIFICATION_EMAIL`** when staff mail should differ from the SMTP login/from address. Each message links to `{FRONTEND_PUBLIC_URL}/admin/bookings/{id}`. Sends are deduplicated per booking via `booking_events`.
